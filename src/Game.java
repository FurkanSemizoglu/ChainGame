...
public class Game {
   public enigma.console.Console cn = Enigma.getConsole("Chain Game",100,26,18,0 );
   ...
   Game() throws Exception {   // --- Contructor
    		 ...
    		 if(i % 2 == 0) {
    			 if(j % 2== 0) {
            		...
            		// Integer.toString(number).charAt(0);
            		 map[i][j] = Integer.toString(number).charAt(0);  
        		 }  	
    		 }
    		 else {
    			 map[i][j] =' ';
      ...
      for(int i = 40; i <91;i++) {
    	  cn.getTextWindow().setCursorPosition(i, 6);
